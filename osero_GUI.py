...
def error_decision(arrangement_lst,mouseX,mouseY,turn,mBtn1):
    x = 10
    y = 10
    if mBtn1 == 1:
...
def main(screen):
    ...
    pygame.draw.rect(screen, GREEN, [650,50,400,400])
    for y in range(8):
        for x in range(8):
            pygame.draw.rect(screen,BLACK, [50*(x+1)+600,50*(y+1),50,50],5)
    ...
    while True: #ゲーム開始
        ...
        x,y = error_decision(lst,mouseX,mouseY,turn,mBtn1)
        yx = str(y)+str(x)
